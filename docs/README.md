...
> Git 说明：体积较大且通常不修改的厂商原件、安装包、视频、镜像、固件和资料包只保留在本地资料库，不纳入 Git。仓库只追踪公开导航、架构/开发文档、项目事实、工程决策、待验证事项和接口文档；克隆仓库后如需硬件开发，必须另行取得原始资料并重建索引。

## 公开展示与开发入口

| 文档 | 用途 |
|---|---|
| [项目复盘](PORTFOLIO.md) | 面向作品集读者说明问题、方案、难点与成果 |
| [系统架构](ARCHITECTURE.md) | 说明模块、数据流、任务复用和安全边界 |
| [开发与运行指南](DEVELOPMENT.md) | 环境、离线测试、规划与真实执行检查 |
| [开源发布检查](OPEN_SOURCE_CHECKLIST.md) | 许可、第三方代码、隐私与仓库清理 |
| [第三方代码与许可说明](../THIRD_PARTY_NOTICES.md) | MIT 适用范围与第三方再分发风险 |
