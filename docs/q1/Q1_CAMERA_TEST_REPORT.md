...
- 33/33 自动测试通过（包含Snapshot规划与逐片复检）
- Python 编译检查通过
...
| 最大顶点误差 | 1.3663501715929642 | 差异小于 0.05 cm |
| 四块中心坐标（6位小数） | 基线 | 各坐标差异小于 0.05 cm |

这证明当前离线样本仍为 4/4 且拼合通过；微小坐标差异来自纸框从水平矩形改为真实四边形，不代表所有真实摄像头场景都已验证。
