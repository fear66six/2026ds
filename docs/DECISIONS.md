...
## D-042 Z-15 抓放使用固定缓冲位姿

- 决策：使用固定 `buffer_pose=[175,0,80,-90,0,0,3000]`。第一片执行 `HOME → 吸取 → BUFFER → 释放`；后续各片执行 `上一释放点 → BUFFER → 吸取 → BUFFER → 释放`；最后一片释放后返回 HOME。
- 依据：F-032；厂商 `kPickPlaceSequence` 示例确认固定中间位姿的多段搬运编排可由 NexArm 笛卡尔位姿序列实现。缓冲位姿的具体六维值和 3000 ms 时长是用户项目决策。
- 影响范围：替换此前动态插值的两段中间点；HOME、吸取点、释放点、纸面映射和电磁铁租约不变。不恢复历史 Z226/Z250 航点，不发送全局加速度或分轴增量命令。
- 当前状态：本地 Q1 回归通过；固定 BUFFER 的真实可达性和完整四片轨迹尚未实机验证，未同步 Jetson。

## D-041 NexArm 串口打开后立即用 3000 ms 发送 HOME

- 决策：正式 `q1.main run` 的启动顺序改为 `NexArm open → HOME(3000 ms) → K230 初始化 → STM32 初始化 → 拍照规划`。HOME 下发并入 NexArm 初始化，控制器不再重复发送 HOME；D-043 已删除 HOME 后冗余的固件/坐标查询，并在 HOME 命令之后加入目标关节到位等待。
...
## D-043 Q1 以目标关节到位触发电磁铁并在缓冲转移中完成 roll

- 决策：覆盖 D-037 的按动作时长直接推进。正式 Q1 先发送笛卡尔位姿，再读取 `CMD_IKINE_RESULT_GET` 给出的同目标六轴脉冲，并以 `CMD_GET_REAL_JOINT_ANGLES` 返回的六轴真实关节脉冲作为到位反馈；不再使用会在已确认物理运动期间滞留的 `CMD_GET_CUR_COORDS` 及其附带脉冲。最大目标误差不超过 40、相邻样本变化不超过 2 脉冲，并连续满足 3 次才进入下一阶段。因此吸合只发生在吸取位关节到达并停止后，断电只发生在释放位关节到达并停止后。动作时长仍作为控制器轨迹字段，不再单独构成到位证据；启动 HOME 仍是 NexArm 串口打开后的第一条协议命令。
- 决策：生产运动等待不再设置总时长超时，短暂查询超时只重试，不以陈旧的笛卡尔反馈终止已实际执行的动作。HOME 关节到位后进行唯一一次正式拍照；场景识别和规划成功同时确认该观察画面可用。
- 决策：HOME 已通过同一目标关节闭环后，不再追加一次 1 秒固件版本/坐标查询。K230 初始化只完成 READY/PING/STATUS 握手，不再先拍摄并丢弃 warmup，也不在适配层重复 health check；紧接着的单张照片就是识别、规划和输出使用的正式图。
- 决策：固定 BUFFER 的 XYZ/Pitch/Claw 和 3000 ms 保持不变。前往当前片吸取点之前，BUFFER 使用当前片的吸取 roll；吸起后前往 BUFFER 时使用当前片的释放 roll，使腕部转向与抬升、横移在同一段动作中完成，BUFFER 到释放点不再承担主要 roll 变化。
- 决策：根据用户对 P3 实机方向的确认，将 `wrist_roll_sign` 从 `1` 改为 `-1`；当前规划约定为正向逆时针、负向顺时针。
- 依据：`docs/NexArm机械臂/1.教程资料/1. 快速入门/02 NexArm出厂程序/Nex_Arm.zip!/Nex_Arm/Global.h`、`system_task_handle.cpp`，`2026E/hardware/nexarm/jetson_to_nexarm/nexarm_sdk.py`，`2026E/q1/executors/nexarm.py::_move_and_wait`、`execute_single_move`，`2026E/q1/wrist.py::choose_wrist_release_roll`，`2026E/q1/config/robot_config.json`（A）；只读报告中六轴实际脉冲连续 20 次零波动（A/D）；用户 2026-07-31 完整流程观察（D）。
- 影响范围：Q1 NexArm 位姿完成判定、电磁铁切换时机和 P1-P4 腕部转向；不修改 HOME、Z=-15、纸面映射、10 mm / 3° 容差、STM32 固件或电磁铁租约。
- 待实机验证：40 脉冲阈值依据已记录 HOME 目标与实机到位脉冲最大约 37 的差值选取；需要在 P1-P4 全流程确认每个目标都能进入阈值。若实际脉冲无法到达，流程会保持等待且不会提前切换电磁铁。

## D-044 Q1 恢复动作时长推进并增加 1 秒机械稳定时间

- 决策：覆盖 D-043 的生产关节闭环。当前机械臂的 `CMD_GET_CUR_COORDS` 与 `CMD_GET_REAL_JOINT_ANGLES` 都未能在正式 HOME 流程中提供可用于推进的到位证据，导致相机初始化前无限等待。正式 Q1 恢复已完成过四片流程的动作时长推进，并在每条 HOME、BUFFER、PICK、RELEASE 指令时长结束后固定等待 1.0 秒再进入下一阶段。
- 时序：HOME/BUFFER 为 `3000 ms + 1.0 s`，PICK/RELEASE 为 `6000 ms + 1.0 s`。吸合发生在 PICK 等待完成后，断电发生在 RELEASE 等待完成后；roll 仍按 D-043 在 PICK 到 BUFFER 的转移中完成。
- 依据：成功完整运行 `20260730_215916_405845` 使用动作时长推进（A）；运行 `20260730_224434_329023`、`20260730_230752_091605` 均在实体 HOME 后停于 `MOVE_TO_OBSERVE`，后者约 39 秒后由用户中断且没有任何实际关节样本写入报告（A+D）。
- 边界：该流程保证不会因陈旧反馈阻塞拍照和完整队列，但时长与稳定等待不能独立证明精确到位。增加的 1 秒用于缓解用户观察到的磁铁切换偏早，不改变 HOME、BUFFER、Z=-15、roll、STM32 租约或规划坐标。
- 当前状态：仅本地修改，按用户要求不上传 Jetson。
...
- 采用：模板分配按 `target_scale` 构造特征；Q1 的五组共享边通过最小位移刚体平移统一留出 `edge_gap_mm=5.0` 法向间距，扩展布局重新居中，最大对应顶点计划距离约 10.69 mm；执行顺序为面积从大到小的 `P4→P3→P2→P1`。吸合后的首个斜向 clearance 继续一次完成最终 roll，当归一化转角超过 90° 时，按 `base_ms*abs(delta)/90` 延长该段，不新增独立旋转动作。
...
- 当前状态：既有规划入口保持通过；新目标几何的五条接缝均为 5 mm、交叠面积为 0、最大对应顶点距离约 10.69 mm，布局中心仍为 `(105,222.75) mm`。第三组 P1 的最终 roll `-154.003°` 及既有 transfer 时序不受本次目标平移修改影响。仅本地修改，尚未同步 Jetson 或执行硬件。
...
- 决策：Q3 不复制或另建 K230、NexArm、STM32、手眼标定和 transfer 驱动。正式入口 `q3.main` 继续使用 Q1 的相机、竖向纸面标定、机器人配置和执行器；纸框检测改由 Q3 的通用有色 A4 检测器负责，扑克牌碎片检测、花纹匹配、矩形求解及 `SingleMovePlan` 适配均留在 Q3。
- 目标布局：横拍图经 Q3 矫正后，原图左侧源区为横向纸面左半区，原图右侧目标区为右半区；进入机械臂规划前交换轴，与 Q1 的标准纸面上半区/下半区及既有标定保持一致。求解后的完整牌保持长边沿标准纸面纵向，并居中放入目标半区；每片按面积从大到小执行。
...
## D-051 公开仓库不追踪未修改的 ARM/ST 支持源码

- 决策：`firmware/*/cmsis/` 与 `firmware/*/startup/` 中确认无项目修改的 ARM/ST 文件只保留为本地构建依赖，通过 `.gitignore` 排除并从 Git 索引取消追踪；公开仓库提供恢复依赖的版本提示，不重新分发这些副本。
- 边界：包含项目上电安全修改的 `src/system_stm32f10x.c` 继续追踪并保留 ST 原声明；包含项目诊断修改的 NexArm SDK 以及来源混合的 `TaskSuite_E/` 不归类为纯第三方副本，本次不取消追踪。
- 依据：相关 CMSIS/启动文件头的 ARM/ST 版权、版本和日期；`docs/interfaces/magnet_control/` 对 `system_stm32f10x.c` 项目安全修改的追溯；NexArm SDK 当前源码及相关项目事实/决策记录。
- 来源类型：第三方源码原文、当前源码与项目决策记录。
- 可信等级：A+D。
- 实机影响：全新克隆默认不能直接构建 STM32 固件，必须先恢复匹配版本的第三方文件；现有本地工作区文件未删除。
- 最后核查时间：2026-09-17。
