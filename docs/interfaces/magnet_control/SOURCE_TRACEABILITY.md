...
| 结论 | 文件 | 代码/日志位置 | 等级 | 备注 |
|---|---|---|---|---|
| USART1 为 115200 8N1、无流控 | [`main.c`](../../../firmware/stm32f103_uart_magnet/src/main.c#L49)；本地记录 `logs/stm32_uart_bootloader/usart1_runtime_test.log`（不随公开仓库分发） | 源码 49–73；本地日志 2 | A+B | 运行期；Bootloader 的 8E1 不适用 |
...
| `MAGNET_OFF -> OK OFF` | 本地记录 `logs/stm32_uart_bootloader/usart1_runtime_test.log` | 3–4 | B | COM15 实测；日志不公开分发 |
| `PING` 10/10 返回 `PONG` | 同上 | 5–24 | B | 真实运行日志 |
| `GET_STATUS -> MAGNET=0 FAULT=0` | 同上 | 25–26 | B | 只证明控制状态 |
| 该次真实日志未发送 `MAGNET_ON` | 本地记录 `logs/stm32_uart_bootloader/USART1_TEST_REPORT.md` | 21–22 | C | 当时负载断开；报告不公开分发 |
...
| 固件 Device ID 0x410、Flash 64 KiB、Bootloader 2.2 | 本地记录 `logs/stm32_uart_bootloader/UART_BOOTLOADER_REPORT.md` | 5–15 | C | Revision 未返回；报告不公开分发 |
| CubeProgrammer Verify 成功 | 本地记录 `logs/stm32_uart_bootloader/FLASHING_REPORT.md` | 30–39 | C | 仅擦除页 0–3；报告不公开分发 |
| 独立回读与构建 BIN 一致 | 同上 | 41–49 | C | 3648 bytes，SHA256 相同 |
| Option Bytes/RDP 未修改 | 同上 | 51–56 | C | 烧录步骤未 Go/Start |
| 原始 Flash 已备份并校验 | 本地记录 `backup/stm32_before_uart_magnet/backup_manifest.json` | 10–27 | C | 65536 bytes，size/SHA256 verified；备份不公开分发 |
...
1. [`DECISIONS.md`](../../DECISIONS.md) 的 D-004 仍写“尚未烧录或执行 PB12 实机输出测试”，但本地 `FLASHING_REPORT.md` 已记录烧录/回读；本次用户又报告输出和负载测试。旧状态已过期，不用于当前接口结论。
...
3. 早期本地 `USART1_TEST_REPORT.md` 明确记录当时没有发送 `MAGNET_ON` 且负载断开；它与之后用户口述的负载验证属于不同阶段，不构成日志证实。
