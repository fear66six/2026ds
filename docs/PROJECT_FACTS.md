...
- 工程处理：搬运改为三段斜向拱形轨迹。第一点位于源到目标 XY 的 25%、Z45；第二点位于 60%、Z75；最后斜向下降到释放 Z-15。roll 同步按 25%/60%/100% 过渡，三段时长各 2000 ms，总搬运时长仍为 6000 ms。
...
- 可信等级：A（运行目标、源码和离线复算）+ C（厂商多段搬运示例）+ D（现场轨迹观察与拱顶参数决策）；是否需要实机验证：需要。
- 离线复算：四片每个搬运段均同时改变 X、Y、Z；没有固定 XY 的纯升降或固定 Z 的水平转运。P1 三段增量为 `(-3.64,-37.34,+60)`、`(-5.10,-52.28,+30)`、`(-5.82,-59.74,-90) mm`。
- 最后核查时间：2026-07-31。
...
- 工程处理：正式入口现改为先打开 NexArm，串口 `open()` 返回后立即把 HOME 作为第一条协议写入；HOME 完成后才读取固件/坐标并初始化相机与 STM32。HOME 独立时长改为 3000 ms，抓放仍为 6000 ms。
