...
class PieceVisual:
    ...
    x0: int
    y0: int
    motion_local_cm: np.ndarray
    motion_local_px: np.ndarray
...
def _paste_piece_texture(
    ...
    local = np.asarray(visual.motion_local_cm, dtype=np.float64)
    ...
    src_local = np.asarray(visual.motion_local_px, dtype=np.float64)
...
def _draw_target_hint(
    ...
    ox, oy = target_origin
    tw = target_size[0] if target_size else 10.0
    th = target_size[1] if target_size else 6.0
    pts = []
    for x, y in [
        (ox, oy),
        (ox + tw, oy),
...
def _build_visuals(
    ...
    for i, piece in enumerate(pieces):
        ...
        local, start_c, end_c, end_angle = decompose_rigid_motion(src_v, tgt_v)
        motion_world_cm = local + np.array(start_c, dtype=np.float64)
        motion_px = np.array(
            [cm_to_px((float(p[0]), float(p[1])), paper) for p in motion_world_cm],
            dtype=np.float64,
        )
        motion_local_px = motion_px - np.array([x0, y0], dtype=np.float64)
        visuals[i] = PieceVisual(
            ...
            x0=x0,
            y0=y0,
            motion_local_cm=local.copy(),
            motion_local_px=motion_local_px,
