...
class NexArmClient:
    def __init__(self, port, baudrate=1000000, timeout=0.1, write_timeout=0.1):
        ...
        self.last_rx_diagnostics = {
            "bytes_discarded": 0,
            "request_started_s": None,
            "response_received_s": None,
            "skipped_packets": 0,
        }
    ...
    def flush_input_buffer(self):
        """Discard any unread UART bytes so the next reply cannot be a stale frame."""
        ser = self._ensure_open()
        waiting = int(getattr(ser, "in_waiting", 0) or 0)
        discarded = 0
        if waiting > 0:
            leftover = ser.read(waiting)
            discarded = len(leftover)
        reset = getattr(ser, "reset_input_buffer", None)
        if callable(reset):
            reset()
        self.last_rx_diagnostics["bytes_discarded"] = discarded
        return discarded
    ...
    def request(
        ...
        # Replies have no sequence number. Flushing before a request is the only
        # host-side way to avoid accepting a pre-command CMD_GET_CUR_COORDS frame.
        if flush_before is None:
            flush_before = expect_reply
        discarded = 0
        if flush_before:
            discarded = self.flush_input_buffer()
        ...
        while time.monotonic() < deadline:
            ...
            if expected_ids and packet.packet_id not in expected_ids:
                skipped += 1
                continue
            response_received_s = time.monotonic()
            self.last_rx_diagnostics = {
                "bytes_discarded": discarded,
                "request_started_s": request_started_s,
                "response_received_s": response_received_s,
                "skipped_packets": skipped,
                "latency_s": response_received_s - request_started_s,
            }
            return packet
        self.last_rx_diagnostics = {
            "bytes_discarded": discarded,
    ...
    def set_pose(self, x, y, z, pitch, roll, claw, duration_ms):
        ...
        # Drain any leftover replies so later get_current_coords cannot read a
        # pre-command coordinate frame after this fire-and-forget write.
        self.flush_input_buffer()
        self.request(
            SYSTEM_ID,
            NexArmCommand.CMD_COORDINATE_SET,
            payload,
            expect_reply=False,
            flush_before=False,
        )
    ...
    def get_current_coords(self, timeout=0.5):
        ...
        return NexArmCurrentCoords(
            ...
            meta=dict(self.last_rx_diagnostics),
        )
