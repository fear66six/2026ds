...
from q1.camera_source import open_camera as open_configured_camera
...
def main():
    ...
    parser.add_argument("--camera-fourcc", type=str, default=config.CAMERA_FOURCC)
    parser.add_argument("--benchmark-camera", action="store_true")
    parser.add_argument("--opencv-threads", type=int, default=config.OPENCV_NUM_THREADS)
    ...
    if args.list_cameras:
        list_cameras(backend=args.camera_backend)
        return
    ...
    print(f"OpenCV {cv2.__version__}, internal threads={cv2.getNumThreads()}")
    ...
    if args.benchmark_camera:
        from q1.camera_benchmark import run_benchmark
    ...
    if args.run:
        try:
            cap = open_camera(cam_index, **camera_kwargs)
