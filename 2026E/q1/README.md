...
- `run`：打开 NexArm 后立即发送 3000 ms HOME，再初始化 K230 和 STM32并调用
  同一段单次拍照/规划流程，随后顺序执行“到吸取位 → 吸合 → 搬运 → 到释放位 → 释放”。

吸取和释放高度均为 Z=-15。固定缓冲位姿为
`[175, 0, 80, -90, 0, 0, 3000]`。第一片按
`HOME → 吸取 → BUFFER → 释放` 执行，后续各片按
`上一释放点 → BUFFER → 吸取 → BUFFER → 释放` 执行，最后一片释放后回 HOME。
...
当前固定构图中，`detect_paper` 通过左右两个黑色半区和中央白色分界线恢复
横放 A4。上边或下边超出画面时，按完整长边和 A4 固定比例向画外补齐短边，
避免将单个右半区误认成整张纸或压缩纸面坐标。
