"""Q1 top-level workflow: one observation, one plan, ordered piece moves."""
...
from .models import ExecutionResult, PieceMove, SceneAnalysis
from .runtime_config import Q1RuntimeConfig
from .state_machine import Q1State, Q1StateMachine, StateEvent
from .workflow import capture_and_plan, jsonable
...
class Q1Controller:
    ...
    def _capture_and_plan(self) -> SceneAnalysis:
        states = {
    ...
    def _execute_move_queue(self) -> None:
        for move_index, move in enumerate(self.move_queue):
            ...
            if not result.ok:
                raise RuntimeError(
                    f"MOVE_FAILED: template={move.template_id}, reason={result.reason}"
                )
    ...
    def run(self) -> SceneAnalysis:
        ...
        try:
            ...
            self.recorder.write(
                ...
                {
                    "completed": True,
                    "completion_basis": "piece move queue exhausted",
