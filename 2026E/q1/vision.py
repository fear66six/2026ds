...
class PaperFrame:
    ...
    _px_to_plane_matrix: np.ndarray = field(init=False, repr=False, compare=False)
    _plane_to_px_matrix: np.ndarray = field(init=False, repr=False, compare=False)
    _roi_cache: dict = field(default_factory=dict, init=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        self.corners_px = np.asarray(self.corners_px, dtype=np.float32).reshape(4, 2)
        plane = _a4_cm_plane_corners(self.landscape_in_image)
        self._px_to_plane_matrix = cv2.getPerspectiveTransform(self.corners_px, plane)
        self._plane_to_px_matrix = cv2.getPerspectiveTransform(plane, self.corners_px)


_KERNEL_ELLIPSE_5 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
_KERNEL_RECT_7 = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7))
_KERNEL_RECT_9 = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 9))
...
def _detect_outer_mat_frame(gray: np.ndarray) -> Optional[np.ndarray]:
    ...
    dark = cv2.morphologyEx(dark, cv2.MORPH_CLOSE, _KERNEL_RECT_9, iterations=2)
...
def _inner_roi_mask(paper: PaperFrame, shape: Tuple[int, ...]) -> np.ndarray:
    ...
    h, w = shape[:2]
    cache_key = (h, w, round(float(paper.px_per_cm), 4), float(config.MAT_INSET_CM))
    cached = paper._roi_cache.get(cache_key)
    ...
    paper._roi_cache.clear()
    paper._roi_cache[cache_key] = mask
    return mask
...
def _detect_paper_dark(frame: np.ndarray) -> Optional[PaperFrame]:
    ...
    if best is None:
        blur = cv2.GaussianBlur(gray, (5, 5), 0)
        _, dark = cv2.threshold(blur, config.BLACK_PAPER_GRAY_THRESH, 255, cv2.THRESH_BINARY_INV)
        dark = cv2.morphologyEx(dark, cv2.MORPH_CLOSE, _KERNEL_RECT_7, iterations=2)
        contours, _ = cv2.findContours(dark, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def _px_to_cm(pt_px: np.ndarray, paper: PaperFrame) -> Tuple[float, float]:
    ...
    out = cv2.perspectiveTransform(pt, paper._px_to_plane_matrix)[0, 0]
...
def contour_to_cm(contour: np.ndarray, paper: PaperFrame) -> np.ndarray:
    """轮廓像素点映射到 cm 坐标"""
    raw = contour.reshape(-1, 2).astype(np.float32).reshape(1, -1, 2)
    plane = cv2.perspectiveTransform(raw, paper._px_to_plane_matrix)[0]
...
def draw_overlay_live(
    ...
    *,
    ...
    target_px = cm_points_to_px(target_pts_cm, paper)
    cv2.polylines(out, [target_px.astype(np.int32)], True, (0, 255, 0), 1)
...
def draw_overlay(
    ...
    target_px = cm_points_to_px(target_pts_cm, paper)
    cv2.polylines(out, [target_px.astype(np.int32)], True, (0, 255, 0), 2)
    ...
    if assignments:
        for asg in assignments:
            if len(asg.target_vertices_cm) == 0:
                continue
            ghost = cm_points_to_px(asg.target_vertices_cm, paper)
            cv2.polylines(out, [ghost.astype(np.int32)], True, (0, 200, 0), 1, cv2.LINE_AA)
    return out
...
def cm_points_to_px(points_cm: np.ndarray, paper: PaperFrame) -> np.ndarray:
    """批量将标准 A4 cm 坐标映射到像素坐标。"""
    points = np.asarray(points_cm, dtype=np.float32).reshape(-1, 2)
    plane = points[:, [1, 0]] if paper.landscape_in_image else points
    return cv2.perspectiveTransform(
        plane.reshape(1, -1, 2), paper._plane_to_px_matrix
    )[0]
...
def _cm_to_px(pt_cm: Tuple[float, float], paper: PaperFrame) -> Tuple[float, float]:
    ...
    out = cv2.perspectiveTransform(pt, paper._plane_to_px_matrix)[0, 0]
