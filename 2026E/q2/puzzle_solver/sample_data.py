...
def board_left_scattered_four_piece_rectangle() -> list[Piece]:
    """Move the board sample wholly left of a vertical 148.5 mm divider."""

    pieces: list[Piece] = []
    target_centers_x = (35.0, 105.0, 35.0, 105.0)
    for piece, target_center_x in zip(
        board_scattered_four_piece_rectangle(),
        target_centers_x,
    ):
        offset_x = target_center_x - piece.centroid.x
        pieces.append(
            Piece(
                piece.id,
                [(vertex.x + offset_x, vertex.y) for vertex in piece.vertices],
            )
        )
    return pieces
