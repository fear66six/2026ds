...
def plot_pieces(
    ...
    placed = [
        PlacedPiece.from_piece(piece, transform=_identity_transform()) for piece in pieces
    ]
...
def _board_solution_vertices(
    ...
    """Rotate the solved assembly as a whole and center it on the board."""
...
def plot_board_solution(
    ...
    """Render scattered input above a divider and assembly below it.
    ...
    if not 0.0 < divider_y_mm < paper_height:
        raise ValueError("divider must lie inside the paper")
    if axes is None:
        figure_height = 10.0
    ...
    solved_polygons = _board_solution_vertices(solution, target_center)
    ...
    if (
        ...
        or min(solved_ys) <= divider_y_mm
        or max(solved_ys) > paper_height
    ):
        raise ValueError("solved rectangle does not fit below the divider")
...
def animate_board_solution(
    ...
    target_polygons = {
        ...
        for placed, coordinates in _board_solution_vertices(solution, target_center)
    }
...
def _identity_transform():  # type: ignore[no-untyped-def]
